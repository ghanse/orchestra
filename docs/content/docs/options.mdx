...
The `profile` phase emits `<output_dir>/metadata/profile_report.csv` (default `./orchestra_output/metadata/profile_report.csv`), one row per pipeline with the following columns:
...
When running with workspace auth (Genie Code, or a configured Databricks profile) the `prepare`
