...
def _web_activity_notebook(task_key: str, activity_name: str, task_ir: dict[str, Any]) -> str:
    ...
    # Normalise the URL (may contain {{job.parameters.*}} refs)
    ...
    if body_raw:
        if isinstance(body_raw, dict) and body_raw.get("type") == "Expression":
            expr = body_raw.get("value", "")
            body_code = f"# ADF expression: {expr}\n# TODO: Translate the body expression to Python\npayload = {{}}"
...
def _normalize_base_parameters(
    ...
    *,
    ...
    for k, v in params.items():
        ...
        if "dbutils.widgets.get" in val or "dbutils.jobs.taskValues" in val:
            _warn(
                task_key,
                f"Parameter `{k}` contains a computed expression that cannot be "
...
def _pipeline_dict_to_workflow(pipeline_dict: dict[str, Any]) -> PreparedWorkflow:
    ...
    for p in pipeline_dict.get("parameters") or []:
...
def _task_ir_to_dab(task_ir: dict[str, Any]) -> dict[str, Any]:
    ...
    if task_type_name == "ForEachActivity":
        ...
        nb_rel_path = f"notebooks/{task_key}.py"
        task["notebook_task"] = {
            "notebook_path": f"../src/{nb_rel_path}",
        }
        ...
        # Attempt to download the actual notebook from the workspace
        ...
        if task_ir.get("parameters"):
            # Resolve any remaining expression dicts to strings
            resolved_params = []
            for p in task_ir["parameters"]:
                resolved_params.append(_normalize_value_import(p))
        ...
        if task_ir.get("parameters"):
            ...
            for k, v in task_ir["parameters"].items():
                val = _normalize_value_import(v)
                if "dbutils.widgets.get" in val or "dbutils.jobs.taskValues" in val:
                    _warn(
                        task_key,
                        f"Parameter `{k}` contains a computed expression that cannot be "
        ...
        # Reconstruct typed IR and call the appropriate code generator
        nb_rel_path = f"notebooks/{task_key}.py"
        ir_obj = _reconstruct_ir(task_ir)
        if ir_obj is not None:
            ...
            gen_fn = _generators[task_type_name]
            content = gen_fn(ir_obj)
        else:
            content = _placeholder_notebook(task_key, activity_name, task_type_name)

        task["notebook_task"] = {"notebook_path": f"../src/{nb_rel_path}"}

        # Add base_parameters for types that need them
        if task_type_name == "LookupActivity":
            ...
            if task_ir.get("source_type"):
                bp["source_type"] = task_ir["source_type"]
            if task_ir.get("sink_type"):
                bp["sink_type"] = task_ir["sink_type"]
            if bp:
                task["notebook_task"]["base_parameters"] = bp
            ...
            bp_sv: dict[str, str] = {"variable_name": task_ir.get("variable_name", "")}
            if task_ir.get("value_kind") in ("literal", "dab_ref"):
                bp_sv["value"] = _normalize_value_import(task_ir.get("variable_value", ""))
            task["notebook_task"]["base_parameters"] = bp_sv
        ...
        nb_rel_path = f"notebooks/{task_key or 'placeholder'}.py"
        task["notebook_task"] = {"notebook_path": f"../src/{nb_rel_path}"}
        notebooks.append(
            DabNotebook(
                relative_path=nb_rel_path,
        ...
        nb_rel_path = f"notebooks/{task_key or 'unknown'}.py"
        task["notebook_task"] = {
            "notebook_path": f"../src/{nb_rel_path}",
        }
        notebooks.append(
...
def _handle_for_each(
    ...
    if len(inner_activity_dicts) == 1:
        ...
        # Convert each inner activity to a DAB task dict
